...
void ofxOfeliaTextBuf::readMethod(t_symbol *s, int argc, t_atom *argv)
{
           ...
           *argv->a_w.w_symbol->s_name == '-')
    {
        ...
        if (!std::strcmp(flag, "-c"))
            ...
            pd_error("ofelia read: unknown flag '%s'", flag);
        argc--; argv++;
    }
    ...
    {
        pd_error("ofelia read: no file name given");
        return;
    }
    if (argc)
    {
    ...
    if (binbuf_read_via_canvas(dataPtr->binbuf, fileName->s_name, dataPtr->canvas, cr))
        pd_error("%s: read failed", fileName->s_name);
    senditup();
    dataPtr->lua.doText();
}
...
void ofxOfeliaTextBuf::writeMethod(t_symbol *s, int argc, t_atom *argv)
{
           ...
           *argv->a_w.w_symbol->s_name == '-')
    {
        ...
        if (!std::strcmp(flag, "-c"))
            ...
            pd_error("ofelia write: unknown flag '%s'", flag);
        argc--; argv++;
    }
    if (argc && argv->a_type == A_SYMBOL)
    {
    ...
    {
        pd_error("ofelia write: no file name given");
        return;
    }
    if (argc)
    {

...
void ofxOfeliaTextBuf::readMethod(t_symbol *s, int argc, t_atom *argv)
{
           ...
           *argv->a_w.w_symbol->s_name == '-')
    {
        ...
        if (!std::strcmp(flag, "-c"))
            ...
            pd_error(NULL, "ofelia read: unknown flag '%s'", flag);
        argc--; argv++;
    }
    ...
    {
        pd_error(NULL, "ofelia read: no file name given");
        return;
    }
    if (argc)
    {
...
void ofxOfeliaTextBuf::writeMethod(t_symbol *s, int argc, t_atom *argv)
{
           ...
           *argv->a_w.w_symbol->s_name == '-')
    {
        ...
        if (!std::strcmp(flag, "-c"))
            ...
            pd_error(NULL, "ofelia write: unknown flag '%s'", flag);
        argc--; argv++;
    }
    if (argc && argv->a_type == A_SYMBOL)
    {
    ...
    {
        pd_error(NULL, "ofelia write: no file name given");
        return;
    }
    if (argc)
    {
